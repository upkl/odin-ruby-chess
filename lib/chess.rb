...
class Board
  ...
  def initialize(fen="rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1")
    ...
    @en_passant = nil if @en_passant == "-" || @en_passant == "-"
    @castling = "" if @castling == "-" || @castling == "-"
  ...
  def to_fen
    ...
    (1..8).each do |rank|
      ...
      code += empty_seq.to_s if empty_seq > 0
      figures << code
    end
  ...
  def check?(color: @active_color)
    king = @squares.filter do |pos, square|
  ...
  def attacked?(square, color)
    ...
    possible_attackers.reject!{ |m| m.class == Symbol }
    possible_attackers.filter!{ |m| m[:dest] == square }
    possible_attackers.reject!{ |m| m[:piece].type == :pawn && m[:origin][0] == m[:dest][0] }
    not possible_attackers.empty?
  end

  def possible_moves(given_origin: nil, color: @active_color, attack: nil)
    if given_origin.nil?
      possible_origins = (@squares.filter { |pos, square| (not square.piece.nil?) && square.piece.color == color }).keys
    else
      possible_origins = [given_origin]
    end
    result = []
      ...
      if piece.type == :king
        ...
        if color == :white
          if @castling.include?("Q")
            if @squares["b1"].piece.nil? \
              && @squares["c1"].piece.nil? && @squares["d1"].piece.nil?
              result << :white_queenside_castling
            end
          end
          if @castling.include?("K")
            if @squares["f1"].piece.nil? && @squares["g1"].piece.nil?
              result << :white_kingside_castling
            end
          end
        elsif color == :black
          if @castling.include?("q")
            if @squares["b8"].piece.nil? \
              && @squares["c8"].piece.nil? && @squares["d8"].piece.nil?
              result << :black_queenside_castling
            end
          end
          if @castling.include?("k")
            if @squares["f8"].piece.nil? && @squares["g8"].piece.nil?
        ...
        next_moving = next_square(origin,
          color == :white ? [0, 1] : [0, -1])
        if @squares[next_moving].piece.nil?
          tmp_result << next_moving
          if origin[1] == (color == :white ? "2" : "7")
            next_moving_2 = next_square(origin,
              color == :white ? [0, 2] : [0, -2])
            tmp_result << next_moving_2 if @squares[next_moving_2].piece.nil?   
          end
        end
        next_capture_1 = next_square(origin,
          color == :white ? [1, 1] : [1, -1])
        can_capture_1 = next_capture_1 && (
          (@squares[next_capture_1].piece && @squares[next_capture_1].piece.color != color) \
          || next_capture_1 == attack \
          || next_capture_1 == @en_passant)
        tmp_result << next_capture_1 if can_capture_1
        next_capture_2 = next_square(origin,
          color == :white ? [-1, 1] : [-1, -1])
        can_capture_2 =  next_capture_2 && (
          (@squares[next_capture_2].piece && @squares[next_capture_2].piece.color != color) \
          || next_capture_2 == attack \
          || next_capture_2 == @en_passant)
        tmp_result << next_capture_2 if can_capture_2
      end
  ...
  def move_safe_for_king?(args)
    if args == :white_kingside_castling
      not (attacked?("e1", :black) || attacked?("f1", :black) || attacked?("g1", :black))
    elsif args == :white_queenside_castling
      not (attacked?("e1", :black) || attacked?("d1", :black) || attacked?("c1", :black))
    elsif args == :black_kingside_castling
      not (attacked?("e8", :white) || attacked?("f8", :white) || attacked?("g8", :white))
    elsif args == :black_queenside_castling
      not (attacked?("e8", :white) || attacked?("d8", :white) || attacked?("c8", :white))
    else
      b1 = clone
      b1.move(args)
      king = b1.squares.filter do |pos, square|
        square.piece && square.piece.color == args[:piece].color && square.piece.type == :king
      end
      king = king.keys[0]
      not b1.attacked?(king, args[:piece].color == :white ? :black : :white )
    end      
  end
  ...
  def move(args)
    ...
    if args == :white_kingside_castling
      ...
      piece, origin, dest = args[:piece], args[:origin], args[:dest]
      ...
      if @castling != ""
        if piece == WHITE_KING
          @castling.sub!("K", "")
          @castling.sub!("Q", "")
        elsif piece == BLACK_KING
          @castling.sub!("k", "")
          @castling.sub!("q", "")
        elsif piece == WHITE_ROOK
          if origin == "a1"
            @castling.sub!("Q", "")
          elsif origin == "h1"
            @castling.sub!("K", "")
          end
        elsif piece == BLACK_ROOK
          if origin == "a8"
            @castling.sub!("q", "")
          elsif origin == "h8"
            @castling.sub!("k", "")
      ...
      if piece.type == :pawn
        if piece.color == :white && origin[1] == "2" and dest[1] == "4"
          @en_passant = origin[0] + "3"
        elsif  piece.color == :white && origin[1] == "7" and dest[1] == "5"
          @en_passant = origin[0] + "6"
        else
          @en_passant = nil
        end
      else
  ...
  def decode_move(move_string)
    ...
    if move_string == "OO" or move_string == "00"
      if @active_color == :white
        return :white_kingside_castling if possible_moves.include?(:white_kingside_castling)
      else
        return :black_kingside_castling if possible_moves.include?(:black_kingside_castling)
      end
      return nil
    elsif move_string == "OOO" or move_string == "000"
      if @active_color == :white
        return :white_queenside_castling if possible_moves.include?(:white_queenside_castling)
      else
        return :black_queenside_castling if possible_moves.include?(:black_queenside_castling)
      end      
      return nil
    end
    piece = move_string[0]
    ...
    if origin.length == 0
      possible = possible_moves.filter { |m| (not (m.class == Symbol)) && m[:dest] == dest && m[:piece] == piece }
    elsif origin.length == 1 && origin.between?("a", "h")
      possible = possible_moves.filter { |m| (not (m.class == Symbol)) && m[:origin][0] == origin && m[:dest] == dest && m[:piece] == piece }
    elsif origin.length == 1 && origin.between?("1", "8")
      possible = possible_moves.filter { |m| (not (m.class == Symbol)) && m[:origin][1] == origin && m[:dest] == dest && m[:piece] == piece }
    elsif origin.length == 2
      possible = possible_moves.filter { |m| (not (m.class == Symbol)) && m[:origin] == origin && m[:dest] == dest && m[:piece] == piece }
    else
      return nil
    end
